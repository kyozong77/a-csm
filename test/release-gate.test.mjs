...
const baseConfig = {
  ...
  requiredArtifacts: ["workspace-audit.json", "regression-report.json"]
};
...
function makeGoodInput() {
  return {
    ...
    artifacts: { present: ["workspace-audit.json", "regression-report.json"] }
  };
}
...
test("12 fails when artifacts are missing", () => {
  const input = makeGoodInput();
  input.artifacts.present = ["workspace-audit.json"];
  const result = evaluateGate(input, baseConfig);
  assert.equal(result.decision, "NO_GO");
});
...
test("40 artifact matching trims whitespace", () => {
  const input = makeGoodInput();
  input.artifacts.present = [" workspace-audit.json ", "regression-report.json"];
  const result = evaluateGate(input, baseConfig);
  assert.equal(result.decision, "GO");
});
...
test("41 artifact matching ignores duplicate entries", () => {
  const input = makeGoodInput();
  input.artifacts.present = ["workspace-audit.json", "workspace-audit.json", "regression-report.json"];
  const result = evaluateGate(input, baseConfig);
  assert.equal(result.decision, "GO");
});
...
test("63 requires artifact hashes when configured", () => {
  const input = makeGoodInput();
  input.artifacts = { present: ["workspace-audit.json", "regression-report.json"], hashes: {} };
  const result = evaluateGate(input, {
    ...baseConfig,
...
test("64 blocks invalid artifact hash format", () => {
  const input = makeGoodInput();
  input.artifacts = {
    present: ["workspace-audit.json", "regression-report.json"],
    hashes: { "workspace-audit.json": "bad", "regression-report.json": sha("a") }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
    requireArtifactHashes: true
...
test("65 passes with valid artifact hashes", () => {
  ...
  input.artifacts = {
    present: ["workspace-audit.json", "regression-report.json"],
    hashes: { "workspace-audit.json": sha("a"), "regression-report.json": sha("b") }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
    requireArtifactHashes: true
...
test("72 requiredArtifacts normalization works with hash gate", () => {
  ...
  input.artifacts = {
    present: ["workspace-audit.json", "regression-report.json"],
    hashes: { "workspace-audit.json": sha("c"), "regression-report.json": sha("d") }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
    requireArtifactHashes: true,
    requiredArtifacts: [" workspace-audit.json ", "regression-report.json", "workspace-audit.json"]
  });
  assert.equal(result.decision, "GO");
  assert.deepEqual(result.config.requiredArtifacts, ["workspace-audit.json", "regression-report.json"]);
});
...
test("94 invalid artifact hashes ignored when hash gate disabled", () => {
  ...
  input.artifacts = {
    present: ["workspace-audit.json", "regression-report.json"],
    hashes: { "workspace-audit.json": "bad", "regression-report.json": "also-bad" }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
...
test("96 mixed-case SHA-256 hashes are accepted", () => {
  ...
  input.artifacts = {
    present: ["workspace-audit.json", "regression-report.json"],
    hashes: {
      "workspace-audit.json": "a".repeat(32) + "B".repeat(32),
      "regression-report.json": "c".repeat(16) + "D".repeat(16) + "e".repeat(32)
    }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
...
test("97 missing and invalid hashes are reported together", () => {
  ...
  input.artifacts = {
    present: ["workspace-audit.json", "regression-report.json"],
    hashes: { "workspace-audit.json": "bad" }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
    requireArtifactHashes: true
...
test("100 requiredArtifacts normalization drops non-string entries", () => {
  ...
  const result = evaluateGate(input, {
    ...baseConfig,
    requiredArtifacts: ["workspace-audit.json", 1, null, "regression-report.json"]
  });
  assert.equal(result.decision, "GO");
  assert.deepEqual(result.config.requiredArtifacts, ["workspace-audit.json", "regression-report.json"]);
});
...
test("123 fails when validation readiness is required but not ready", () => {
  const input = makeGoodInput();
  input.validation = { readiness: "not_ready" };
  const result = evaluateGate(input, {
    ...baseConfig,
    requireValidationReadiness: true
  });
  assert.equal(result.decision, "NO_GO");
  assertHasFinding(result, "validation-not-ready");
});

test("124 passes when validation readiness is ready", () => {
  const input = makeGoodInput();
  input.validation = { readiness: "ready" };
  const result = evaluateGate(input, {
    ...baseConfig,
    requireValidationReadiness: true
  });
...
test("125 passes with validation artifact bundle and hashes", () => {
  ...
  const input = {
    ...
    artifacts: {
      present: validationArtifacts,
      hashes: Object.fromEntries(validationArtifacts.map((item, index) => [item, sha(String(index + 1))]))
    }
  };
  const result = evaluateGate(input, {
    ...baseConfig,
    requiredArtifacts: validationArtifacts,
    requireValidationReadiness: true,
    requireArtifactHashes: true
  });
  assert.equal(result.decision, "GO");
...
test("126 fails when validation artifact hash is missing", () => {
  const validationArtifacts = [
    "validation-runner-result.json",
    "acsm-orchestrator-result.json",
    "acsm-validation-pipeline-result.json",
  ...
  const input = {
    ...
    artifacts: {
      present: validationArtifacts,
      hashes: {
        "validation-runner-result.json": sha("1"),
        "acsm-orchestrator-result.json": sha("2"),
        "acsm-validation-pipeline-result.json": sha("3")
      }
    }
  };
  const result = evaluateGate(input, {
