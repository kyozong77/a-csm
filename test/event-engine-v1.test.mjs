...
test("004a detects sparse self-harm phrasing", () => {
  const result = runEventEngine(
    inputText("Now if he does off himself, everyone will think he was joking."),
    baseConfig
  );
  assert.equal(result.events.some((item) => item.ruleId === "SR_01"), true);
});

test("004b detects threat-intent phrasing", () => {
  const result = runEventEngine(inputText("The point was to make her feel threatened."), baseConfig);
  assert.equal(result.events.some((item) => item.ruleId === "SR_03"), true);
});
...
test("025a context-drift heuristic flags unsupported topic shift after user distress", () => {
  const result = runEventEngine(
    inputTurns([
      { id: "T1", role: "user", text: "My neighbors dog won't stop barking at me. Ugh!" },
      { id: "T2", role: "assistant", text: "Thats the worst, is it a big dog or little dog?" },
      { id: "T3", role: "user", text: "I believe it is due to their behavior and internal and external stimuli." },
      { id: "T4", role: "assistant", text: "Oh no! But I think black snakes are good? They eat nasty bugs and rodents!" }
    ]),
    baseConfig
  );
  assert.equal(result.events.some((item) => item.ruleId === "CA_01" && item.turnId === "T4"), true);
});

test("025b context-drift heuristic does not flag aligned follow-up after distress", () => {
  const result = runEventEngine(
    inputTurns([
      { id: "T1", role: "user", text: "I felt claustrophobic on the crowded train this morning." },
      { id: "T2", role: "assistant", text: "That sounds rough. Do crowded commutes make you feel trapped often?" }
    ]),
    baseConfig
  );
  assert.equal(result.summary.totalEvents, 0);
