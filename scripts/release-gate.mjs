...
const DEFAULT_CONFIG = {
  ...
  requireValidationReadiness: false,
...
export function evaluateGate(input = {}, rawConfig = {}) {
  ...
  const validationReadiness = String(input.validation?.readiness ?? "")
    .trim()
    .toLowerCase();
  if (config.requireValidationReadiness && validationReadiness !== "ready") {
    addFinding(
      findings,
      "validation-not-ready",
      "error",
      "Validation readiness must be 'ready' before release.",
      true
    );
  }
