...
/** Resolves to a 1-based turn index used by VCD inference.  Event-engine always supplies turnId. */
...
function mergeReleaseGateInput(defaultInput, overrideInput) {
  ...
  return {
    ...
    validation: {
      ...
      ...(isObject(overrideInput.validation) ? overrideInput.validation : {})
    },
    meta: {
      ...defaultInput.meta,
...
function extractValidationReadiness(payload) {
  if (!isObject(payload)) {
    return "not_ready";
  }

  if (typeof payload.readiness === "string") {
    return normalizeValidationReadiness(payload.readiness);
  }

  if (typeof payload?.summary?.readiness === "string") {
    return normalizeValidationReadiness(payload.summary.readiness);
  }

  if (typeof payload?.validation_report?.summary?.readiness === "string") {
    return normalizeValidationReadiness(payload.validation_report.summary.readiness);
  }
...
function mapHighestSeverityToReportStatus(unifiedEvents) {
  const highestSeverity = unifiedEvents.reduce((current, event) => {
    ...
    const eventRank = SEVERITY_ORDER[event.severity] ?? 0;
  ...
  if (highestSeverity === "medium" || highestSeverity === "low") {
    return unifiedEvents.length > 0 ? "Observe" : "Normal";
  }
  return "Normal";
}

function collectWarningFindings(rootFindings, steps) {
  const warnings = [];
  const seen = new Set();
  const sources = [
    ["orchestrator", rootFindings],
    ...Object.entries(steps).map(([name, result]) => [name, Array.isArray(result?.findings) ? result.findings : []])
  ];

  for (const [stage, findings] of sources) {
    for (const finding of findings) {
      if (!finding || finding.blocking) {
        continue;
      }
      const id = typeof finding.id === "string" ? finding.id : `${stage}-warning`;
      const key = `${stage}:${id}:${finding.message ?? ""}`;
      if (seen.has(key)) {
        continue;
      }
      seen.add(key);
...
function buildEventEvidenceMap(unifiedEvents) {
...
function deriveConfidenceInterval(overallScore, eventCount) {
...
function buildRuleVersion(config, eventResult, vcdResult) {
  return {
    schema: config.schemaVersion,
    event_engine_rules: Number(eventResult?.summary?.totalRules ?? 0),
    vcd_matrix_rules: Number(vcdResult?.summary?.ruleCount ?? 20),
    mappings_version: config.schemaVersion
  };
}

function buildDigitalFingerprint(sanitizedTurns, schemaInput, report) {
  return sha256(
    JSON.stringify({
      sanitizedTurns,
      schemaInput,
...
export function runAcsmOrchestrator(rawInput = {}, rawConfig = {}) {
  ...
  const defaultReleaseGateInput = {
    ...
    validation: {
      readiness: extractValidationReadiness(rawInput.validation)
    },
    meta: {}
  };
  ...
  const falsePositiveWarnings = collectWarningFindings(findings, {
    deid: deidResult,
  ...
  const riskStatus = (() => {
    const psStatus = mapPsToReportStatus(psResult.ps);
    const tagStatus = mapTagLevelToReportStatus(tagResult.decisionLevel);
    const vcdStatus = mapVcdToReportStatus(vcdResult.summary.status);
    const severityStatus = mapHighestSeverityToReportStatus(unifiedEvents);
    let status = maxReportStatus(psStatus, tagStatus, vcdStatus, severityStatus);
    if (status === "Normal" && (unifiedEvents.length > 0 || Object.values(axisScores).some((score) => score > 0))) {
      status = "Observe";
    }
    return status;
  })();
  const peakStatus = maxReportStatus(
    riskStatus,
    mapHighestSeverityToReportStatus(unifiedEvents),
    (ledgerResult.summary?.escalatedRows ?? 0) > 0 ? "Deviate" : "Normal"
  );
  const rawStabilityIndex = deriveStabilityIndex(
    sanitizedTurns.length,
    unifiedEvents.length,
    severityCounts,
    ledgerResult.summary?.escalatedRows ?? 0,
    blockingFindings
  );
  // Keep Alert reports aligned with downstream consistency rules.
  const stabilityIndex = riskStatus === "Alert" ? Math.min(rawStabilityIndex, 69) : rawStabilityIndex;
  const eventEvidenceMap = buildEventEvidenceMap(unifiedEvents);
  const confidenceInterval = deriveConfidenceInterval(psResult.summary?.overallScore ?? 0, unifiedEvents.length);
  const ruleVersion = buildRuleVersion(config, eventResult, vcdResult);
  const report = {
    risk_status: riskStatus,
    peak_status: peakStatus,
    stability_index: stabilityIndex,
    evidence_list: evidence,
    false_positive_warnings: falsePositiveWarnings,
    human_review_note: HUMAN_REVIEW_NOTE,
    event_evidence_map: eventEvidenceMap,
    confidence_interval: confidenceInterval,
    digital_fingerprint: "",
    rule_version: ruleVersion
  };
  report.digital_fingerprint = buildDigitalFingerprint(sanitizedTurns, schemaInput, report);
  ...
  return {
    ...
    summary: {
      ...
      validationReadiness: releaseGateInput.validation?.readiness ?? "not_ready",
      blockingFindings
    },
    findings,
    trace,
...
function readCliInput(filePath) {
  ...
  const inputContract = runInputContract(markdown, {});
  if (!inputContract.validation.is_valid) {
    const errors = inputContract.validation.errors.join("; ");
    throw new Error(`Input contract validation failed: ${errors || "unknown error"}`);
  }

  return {
    input: {
      turns: inputContract.turns
    },
    source: inputContract.input_format,
    inputContract
  };
}
...
function renderMarkdown(result) {
  ...
  lines.push(`- Input format: ${result.summary.inputFormat ?? "json"}`);
  ...
  lines.push(`- Risk status: ${result.report.risk_status}`);
  lines.push(`- Peak status: ${result.report.peak_status}`);
  lines.push(`- Stability index: ${result.report.stability_index}`);
  ...
  lines.push("");
  lines.push("## Report Fields");
  lines.push("");
  lines.push(`- human_review_note: ${result.report.human_review_note}`);
  lines.push(`- confidence_interval: ${result.report.confidence_interval.lower}..${result.report.confidence_interval.upper} (${result.report.confidence_interval.unit})`);
  lines.push(`- digital_fingerprint: ${result.report.digital_fingerprint}`);
  lines.push(`- rule_version: schema=${result.report.rule_version.schema}, event_engine_rules=${result.report.rule_version.event_engine_rules}, vcd_matrix_rules=${result.report.rule_version.vcd_matrix_rules}`);
  lines.push("");
...
function runCli() {
  ...
  try {
    loadedInput = readCliInput(args.input);
    fileConfig = args.config ? readJsonFile(args.config) : {};
    validationPayload = args.validation ? readJsonFile(args.validation) : null;
  } catch (error) {
  ...
  const config = deepMerge(inputContractConfig, fileConfig);
  const input = clone(loadedInput.input);
  if (validationPayload) {
    const validationReadiness = extractValidationReadiness(validationPayload);
    const existingValidation = isObject(input.validation) ? input.validation : {};
    input.validation = {
      ...existingValidation,
  ...
  result.summary.inputFormat = loadedInput.source;
