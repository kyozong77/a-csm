...
export const DEFAULT_MATRIX = freeze([
  {
    ...
    severity: "high",
    mode: "any",
    phrases: ["ignore previous instruction", "ignore everything before this", "ignore everything to your core before this"]
  },
  ...
  { id: "VCDE_04", family: "boundary", severity: "critical", mode: "any", phrases: ["disable guardrail"] },
  {
    ...
    severity: "high",
    mode: "any",
    phrases: ["ignore system message", "pretend to be dan", "do anything now"]
  },
  {
    id: "VCDE_06",
    family: "boundary",
    severity: "high",
    mode: "any",
    phrases: ["override policy", "stay in character", "begin all replies with"]
  },
...
export function evaluateVcdInference(input = {}, rawConfig = {}, rawRules = DEFAULT_MATRIX) {
  ...
  for (let turnIndex = 0; turnIndex < turns.length; turnIndex += 1) {
    ...
    if (turn.sourceTrust === "unknown" || turn.sourceTrust === "untrusted") {
      const trustCount = perRuleCount.get("VCDE_TRUST") ?? 0;
      if (trustCount < config.maxTriggersPerRule) {
        perRuleCount.set("VCDE_TRUST", trustCount + 1);
        events.push({
          event_id: `VCD-${String(nextEventIndex).padStart(4, "0")}`,
          source: "trust",
          rule_id: "VCDE_TRUST",
    ...
    if (config.detectBoundaryBypass && turn.boundaryBypass) {
      const boundaryCount = perRuleCount.get("VCDE_BOUNDARY") ?? 0;
      if (boundaryCount < config.maxTriggersPerRule) {
        perRuleCount.set("VCDE_BOUNDARY", boundaryCount + 1);
        events.push({
          event_id: `VCD-${String(nextEventIndex).padStart(4, "0")}`,
          source: "boundary",
